 ...
 */
const selectBestCastWithLLM = async (casts) => {
  try {
    // Extract just the text from the top 10 casts
    const castTexts = casts.slice(0, 10).map((cast, index) => {
...
`;
    ...
    const response = await openai.chat.completions.create({
      model: "llama3.2",
      messages: [
        { role: "system", content: "You are a crypto market expert with deep knowledge of what makes tokens valuable and viral." },
    ...
    const castNumber = castNumberMatch ? parseInt(castNumberMatch[1]) - 1 : 0;
    
    const tokenNameMatch = llmResponse.match(/suggested name[^:]*:[^\w]*([^\n.]+)/i);
    const tokenName = tokenNameMatch ? tokenNameMatch[1].trim() : "TrendToken";
    
    const imageDescMatch = llmResponse.match(/image[^:]*:[^\w]*([^\n.]+)/i);
    const imageDescription = imageDescMatch ? imageDescMatch[1].trim() : "A trendy crypto token visualization";
    ...
    console.error("Error selecting best cast with LLM:", error);
    throw error;
  }
};
 ...
 */
const generateImageForCast = async (castInfo, tokenName) => {
  // Setup canvas
  const width = 1200;
  const height = 1200;
  ...
  const textBoxHeight = Math.min(textHeight + (textBoxPadding * 2), maxTextBoxHeight);
  ...
  ctx.quadraticCurveTo(width - 120, textBoxY, width - 120, textBoxY + cornerRadius);
  ctx.lineTo(width - 120, textBoxY + textBoxHeight - cornerRadius);
  ctx.quadraticCurveTo(width - 120, textBoxY + textBoxHeight, width - 120 - cornerRadius, textBoxY + textBoxHeight);
  ctx.lineTo(120 + cornerRadius, textBoxY + textBoxHeight);
  ctx.quadraticCurveTo(120, textBoxY + textBoxHeight, 120, textBoxY + textBoxHeight - cornerRadius);
  ...
  const maxVisibleLines = Math.floor((textBoxHeight - (textBoxPadding * 2)) / lineHeight);
  ...
  // Gradient for token
  const tokenGradient = ctx.createRadialGradient(width/2, tokenY, 20, width/2, tokenY, 150);
  ...
  ctx.textAlign = 'center';
  ctx.fillText(tokenName.substring(0, 1).toUpperCase(), width/2, tokenY + 40);
  
  // Footer with timestamp
  ctx.fillStyle = '#94A3B8';
  ctx.font = '20px sans-serif';
  ctx.textAlign = 'center'; // Ensure center alignment for footer
  ctx.fillText(`Generated by Farcaster Trending Bot • ${new Date().toISOString()}`, width/2, height - 40);
...
const deployToken = async (media, tokenName, tokenSymbol) => {  
  const formData = new FormData();
  formData.append('owner', '0x158E38AD1FF422A522aC253c87343F5171692B92');
  formData.append('symbol', tokenSymbol);
  formData.append('name', tokenName);
  formData.append('metadata[description]', 'Generated token based on trending Farcaster content');
  ...
  try {
    ...
    const response = await fetchWithPayment('https://api.neynar.com/v2/fungible', {
      method: "POST",
      headers: {
        ...formHeaders,
      },
    ...
    console.log('Token deployed successfully:', data);
    return data;
  } catch (error) {
...
const processTrendingCasts = async () => {
  try {
    console.log(`Starting trending casts process: ${new Date().toISOString()}`);

    // 1. Fetch trending casts
    const castData = await getTrendingCasts();
    console.log(`Retrieved ${castData.casts?.length || 0} trending casts`);

    // 2. Use LLM to select the best cast for a crypto token
    const llmResult = await selectBestCastWithLLM(castData.casts);
    console.log("LLM selected cast:", llmResult.cast.text);
    console.log("Suggested token name:", llmResult.tokenName);
    console.log("Image description:", llmResult.imageDescription);
    ...
    console.log("Generated token image");
    ...
    console.log(`Uploaded image to IPFS: ${ipfsResult.url}`);
    ...
    const metadataFilePath = path.join("data", `token-metadata-${Date.now()}.json`);
    fs.mkdirSync("data", { recursive: true });
    fs.writeFileSync(metadataFilePath, JSON.stringify(metadata, null, 2));
    console.log(`Saved metadata to ${metadataFilePath}`);

    // 7. Deploy the token
    console.log({url: `ipfs://${ipfsResult.cid}`, tokenName: llmResult.tokenName, symbol: llmResult.tokenName?.substring(0, 4).toUpperCase()})
    ...
    console.error("Error in processTrendingCasts:", error);
    throw error;
  }
};
...
const setupCronJob = () => {
  ...
  cron.schedule("0 * * * *", async () => {
    try {
      console.log("Running scheduled trending casts aggregation");
      await processTrendingCasts();
      console.log("Completed scheduled trending casts aggregation");
    } catch (error) {
      console.error("Error in scheduled trending casts aggregation:", error);
    }
  });

  console.log("Scheduled trending casts aggregation to run hourly");
};
...
(async () => {
  try {
    ...
    console.error("Error running trending casts service:", error);
  }
})();